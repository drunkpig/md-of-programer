...
# 修正 package.json 格式问题（如 bin 路径含多余 ./）
npm pkg fix
```
...
**获取 token（Granular Access Token with 2FA bypass）：**
...
- npm 源若配置为淘宝镜像（`npmmirror.com`），login 和 publish 都需加 `--registry https://registry.npmjs.com`
