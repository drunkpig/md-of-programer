---
description: 发布 npm 包到 npmjs.com 时使用此 skill。用户说"发布/发包/publish npm"时触发。
---
...
```bash
# 确认目录正确
cat package.json   # 或用 Read 工具读取

# 新包：确认包名未被占用（404 = 可用）
npm view <name> --registry https://registry.npmjs.com 2>&1

# 预览打包内容，确认无敏感文件（.env、token 等）
npm pack --dry-run

# 若有 bin 字段，确认文件有可执行权限
chmod +x bin/*.js
...
**安全警告**：告知用户 token 必须通过 `!` 前缀在终端直接运行，**不要粘贴进对话消息**。
...
| 错误码 | 原因 | 处理 |
|--------|------|------|
| `E403 Two-factor` | 缺少 bypass token | 按步骤 3 获取 token |
| `E403 no permission` | 包名属于他人 | 换包名 |
| `E409 Conflict` | 版本已存在 | `npm version patch` |
| `bin script invalid` | bin 路径格式问题 | `npm pkg fix` |

## 注意

- 发布**不可撤销**（72 小时后无法 unpublish）
- npm 源若配置为淘宝镜像，login 也需要加 `--registry https://registry.npmjs.com`
